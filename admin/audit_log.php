...
if (isset($_GET['id'])) {
    ...
    $pageTitle = 'Audit Log Entry #' . $logId;
    ?>
...
<div class="card" style="max-width:700px">
    <div class="info-grid">
        <div class="info-item"><div class="info-label">Log ID</div><div class="info-value"><?= h((string)$log['id']) ?></div></div>
        <div class="info-item"><div class="info-label">Timestamp</div><div class="info-value"><?= h(date('d M Y H:i:s', strtotime($log['created_at']))) ?></div></div>
        <div class="info-item">
            <div class="info-label">User</div>
            <div class="info-value">
                <?php if ($log['user_name']): ?>
                <?= h($log['user_name']) ?>
                <?php if ($log['user_role']): ?><span class="role-badge" style="background:<?= h(ROLE_BADGE_COLORS[$log['user_role']] ?? '#555') ?>;margin-left:0.4rem"><?= h($log['user_role']) ?></span><?php endif; ?>
                <?php else: ?><span class="text-muted">Guest / System</span><?php endif; ?>
            </div>
        </div>
        <?php if ($log['user_email']): ?>
        <div class="info-item"><div class="info-label">Email</div><div class="info-value"><?= h($log['user_email']) ?></div></div>
        <?php endif; ?>
        <div class="info-item">
        ...
        <div class="info-item">
            ...
            <div class="info-value"><?= $log['resource'] ? h($log['resource']) . ($log['resource_id'] ? ' <span class="text-muted">#' . h((string)$log['resource_id']) . '</span>' : '') : '<span class="text-muted">—</span>' ?></div>
        </div>
        <div class="info-item"><div class="info-label">IP Address</div><div class="info-value"><code><?= h($log['ip_address']) ?></code></div></div>
...
<div class="table-container">
    <table>
        ...
        <tr class="clickable-row" data-href="<?= APP_URL ?>/admin/audit_log.php?id=<?= (int)$log['id'] ?>">
